...
main = tkinter.Tk()
main.title("Encrypted Network Traffic Classification Using ResNet and Parallel Network-In-Network Models") #designing main screen
...
def calculateMetrics(algorithm, testY, predict):
    ...
    plt.title(algorithm+" Confusion matrix") g
...
def runNINResNet():
    global nin_model,ResNet50
    global X_train, y_train, X_test, y_test
    #will process packet body
    nin_model=ResNet50(include_top=False, weights='imagenet')#base model
    ...
    predict = np.argmax(predict, axis=1)
    testY = np.argmax(y_test, axis=1)
    calculateMetrics("Propose Resnet50 with NIN Algorithm", testY, predict)
...
def graph():
    df = pd.DataFrame([['Standard CNN','Accuracy',accuracy[0]],['Standard CNN','Precision',precision[0]],['Standard CNN','Recall',recall[0]],['Standard CNN','FSCORE',fscore[0]],
                       ['Propose Resnet50 with NIN Algorithm','Accuracy',accuracy[1]],['Propose Resnet50 with NIN Algorithm','Precision',precision[1]],['Propose Resnet50 with NIN Algorithm','Recall',recall[1]],['Propose Resnet50 with NIN Algorithm','FSCORE',fscore[1]],
...
font = ('times', 16, 'bold')
title = Label(main, text='Encrypted Network Traffic Classification Using ResNet and Parallel Network-In-Network Models')
...
rfButton = Button(main, text="Run ResNet50 with Parallel NIN Algorithm", command=runNINResNet)
